{
  ...
  https://stackoverflow.com/questions/4354071/how-can-i-allow-a-form-to-accept
}
...
procedure TDropTarget.GetFileNames(const dataObj: IDataObject;
  var FileNames: TArray<string>);
var
  i: Integer;
...
begin
  ...
  begin
    ///   This cast needed because HDROP is incorrectly declared as Longint in
    ///  ShellAPI.pas.  It should be declared as THandle which is an unsigned
    ///  integer.  Without this fix the routine fails in top-down memory
...
function TDropTarget.DragEnter(const dataObj: IDataObject; grfKeyState: Integer;
...
begin
  ...
  try
    ...
    FDropAllowed := (Length(FileNames) > 0) and
      FDragDrop.DropAllowed(FileNames);
    SetEffect(dwEffect);
  except
    Result := E_UNEXPECTED;
  end;
